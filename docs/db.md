...
Ref: shopping_list_item.category_id > category.id
...
Table category {
  id int [primary key, increment]
  icon varchar(500) [null, note: "flutter icon"]
  slug text [not null, note: "category names are translated in app, slug serves as a human readable ID"]
